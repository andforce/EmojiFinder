    ...
    public Response intercept(Chain chain) throws IOException {
        if (false) {
            ...
            if (responseBody != null) {
                ...
                Response r = response.newBuilder().body(ResponseBody.create(mediaType, jsonObject.toString())).build();
            ...
            Response response1 = response.newBuilder().body(ResponseBody.create(mediaType, textJsonConverter.convert(response.body().string()))).build();
            System.out.println(">>>" + (System.currentTimeMillis() - start));
            return response1;
        }
    }
